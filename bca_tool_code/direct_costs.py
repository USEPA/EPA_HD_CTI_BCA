...
def tech_package_cost(costs_df, engine, alt, cost_step):
    """
    ...
    """
    pkg_cost_dict_key = (engine, alt, cost_step)
...
def tech_pkg_cost_withlearning(settings, engine, alt, model_year, cost_step, regclass_sales_dict):
    ...
    seedvolume_factor = settings.seedvol_factor_dict[(engine, alt)]['SeedVolumeFactor']
...
def calc_per_veh_direct_costs(settings, regclass_yoy_costs_per_step_dict, averages_dict):
    ...
    for key in averages_dict.keys():
        ...
        if age_id == 0:
            ...
            if alt == 0:
                model_year_cost = regclass_yoy_costs_per_step_dict[(engine, alt, model_year, settings.cost_steps[0])]['DirectCost_AvgPerVeh']
            else:
                model_year_cost = regclass_yoy_costs_per_step_dict[(engine, 0, model_year, settings.cost_steps[0])]['DirectCost_AvgPerVeh']
                for cost_step in settings.cost_steps:
                    if model_year >= int(cost_step):
