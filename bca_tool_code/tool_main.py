...
def main():
    ...
    if settings.calc_cap_costs:
        ...
        gen_fxns.save_dict(
            cap_costs.results, path_of_run_results_folder / 'CAP_bca_tool_all',
            row_header=None, stamp=stamp, index=False
        )
        cap_summary_df = gen_fxns.save_dict_return_df(
            settings.annual_summary_cap.results, path_of_run_results_folder / 'CAP_bca_tool_annual_summary',
            row_header=None, stamp=stamp, index=False
        )
        ...
        gen_fxns.save_dict(
            settings.fleet_cap.sales_and_cumsales_by_start_year,
            path_of_run_results_folder / 'CAP_sales_and_cumulative_sales_by_implementation_year',
            row_header=None, stamp=stamp, index=False
        )
        gen_fxns.save_dict(
            settings.regclass_costs.package_cost_by_step,
            path_of_run_results_folder / 'CAP_package_costs_by_implementation_year',
            row_header=None, stamp=stamp, index=False
        )
        gen_fxns.save_dict(
            settings.wtd_cap_fuel_cpm_dict, path_of_run_results_folder / 'CAP_vmt_weighted_fuel_cpm',
            row_header=None, stamp=stamp, index=True
        )
        gen_fxns.save_dict(
            settings.wtd_def_cpm_dict, path_of_run_results_folder / 'CAP_vmt_weighted_def_cpm',
            row_header=None, stamp=stamp, index=True
        )
        gen_fxns.save_dict(
            settings.wtd_repair_cpm_dict, path_of_run_results_folder / 'CAP_vmt_weighted_repair_cpm',
            row_header=None, stamp=stamp, index=True
        )
        gen_fxns.save_dict(
            settings.estimated_age.estimated_ages_dict, path_of_run_results_folder / 'CAP_estimated_ages',
            row_header=None, stamp=stamp, index=False
        )
        gen_fxns.save_dict(
            settings.emission_repair_cost.repair_cpm_dict, path_of_run_results_folder / 'CAP_repair_cpm_details',
            row_header=None, stamp=stamp, index=False
        )
