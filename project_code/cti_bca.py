...
import time
import project_code
from project_code.get_context_data import GetFuelPrices, GetDeflators
...
def main(bca_tool):
    ...
    if 'Alternative' in bca_tool.moves.columns.tolist():
        bca_tool.moves.rename(columns={'Alternative': 'optionID'}, inplace=True)
    number_alts = len(bca_tool.moves['optionID'].unique())

    # get the fuel price inputs and usd basis for the analysis
    fuel_prices_obj = GetFuelPrices(bca_tool.path_project, bca_tool.aeo_case, 'full name', 'Motor Gasoline', 'Diesel')
    ...
    deflators_obj = GetDeflators(bca_tool.path_project, 'Unnamed: 1', 'Gross domestic product')
    print(deflators_obj)
    gdp_deflators = deflators_obj.calc_adjustment_factors(dollar_basis_analysis)
    cost_steps = [col for col in bca_tool.regclass_costs.columns if '20' in col]
    gen_fxns.convert_dollars_to_analysis_basis(bca_tool.regclass_costs, gdp_deflators, dollar_basis_analysis, [step for step in cost_steps])
    gen_fxns.convert_dollars_to_analysis_basis(bca_tool.def_prices, gdp_deflators, dollar_basis_analysis, 'DEF_USDperGal')
    gen_fxns.convert_dollars_to_analysis_basis(bca_tool.repair_and_maintenance, gdp_deflators, dollar_basis_analysis, 'Value')

    # now get specific inputs from repair_and_maintenance
    inwarranty_repair_and_maintenance_owner_cpm = bca_tool.repair_and_maintenance.at['in-warranty_R&M_Owner_CPM', 'Value']
    atusefullife_repair_and_maintenance_owner_cpm = bca_tool.repair_and_maintenance.at['at-usefullife_R&M_Owner_CPM', 'Value']
    mile_increase_beyond_usefullife = bca_tool.repair_and_maintenance.at['mile_increase_beyond_usefullife', 'Value']
    max_repair_and_maintenance_CPM = bca_tool.repair_and_maintenance.at['max_R&M_Owner_CPM', 'Value']
    typical_vmt_thru_age = bca_tool.repair_and_maintenance.at['typical_vmt_thru_ageID', 'Value']
    emission_repair_share = bca_tool.repair_and_maintenance.at['emission_repair_share', 'Value']
    ...
    warranty_scalers = IndirectCostScalers(bca_tool.warranty_inputs, 'Warranty', bca_tool.indirect_cost_scaling_metric)
    warranty_scalers = warranty_scalers.calc_scalers_absolute()
    usefullife_scalers = IndirectCostScalers(bca_tool.usefullife_inputs, 'RnD', bca_tool.indirect_cost_scaling_metric)
    ...
    warranty_miles_reshaped = gen_fxns.reshape_df(bca_tool.warranty_inputs.loc[bca_tool.warranty_inputs['period'] == 'Miles'], ['optionID', 'regClassID', 'fuelTypeID'],
                                                  [col for col in bca_tool.warranty_inputs.columns if '20' in col], 'modelYearID', 'Warranty_Miles')
    warranty_age_reshaped = gen_fxns.reshape_df(bca_tool.warranty_inputs.loc[bca_tool.warranty_inputs['period'] == 'Age'], ['optionID', 'regClassID', 'fuelTypeID'],
                                                [col for col in bca_tool.warranty_inputs.columns if '20' in col], 'modelYearID', 'Warranty_Age')
    usefullife_miles_reshaped = gen_fxns.reshape_df(bca_tool.usefullife_inputs.loc[bca_tool.usefullife_inputs['period'] == 'Miles'], ['optionID', 'regClassID', 'fuelTypeID'],
                                                    [col for col in bca_tool.usefullife_inputs.columns if '20' in col], 'modelYearID', 'UsefulLife_Miles')
    usefullife_age_reshaped = gen_fxns.reshape_df(bca_tool.usefullife_inputs.loc[bca_tool.usefullife_inputs['period'] == 'Age'], ['optionID', 'regClassID', 'fuelTypeID'],
                                                  [col for col in bca_tool.usefullife_inputs.columns if '20' in col], 'modelYearID', 'UsefulLife_Age')
    ...
    if bca_tool.calc_pollution_effects == 'Y':
        criteria_emission_costs = gen_fxns.read_input_files(bca_tool.path_inputs, bca_tool.input_files_dict['criteria_emission_costs']['UserEntry.csv'], lambda x: 'Notes' not in x)
    ...
    for df in [bca_tool.regclass_costs, bca_tool.regclass_learningscalers, bca_tool.moves, bca_tool.moves_adjustments]:
        df = Fleet(df).define_bca_regclass()
    moves = Fleet(bca_tool.moves).define_bca_sourcetype()

    # adjust MOVES VPOP/VMT/Gallons to reflect what's included in CTI (excluding what's not in CTI)
    moves_adjusted = Fleet(moves).adjust_moves(bca_tool.moves_adjustments)  # adjust (41, 2) to be engine cert only
    ...
    for step, veh in product(cost_steps, alt_rc_ft_vehicles[step]):
        direct_costs_obj = DirectCost(veh, step, bca_tool.regclass_costs, bca_tool.regclass_learningscalers, regclass_sales_dict[step])
        print(direct_costs_obj)
        regclass_costs_dict[veh, step] = direct_costs_obj.pkg_cost_regclass_withlearning(bca_tool.learning_rate)
    ...
    for veh in alt_rc_ft_vehicles:
        ...
        regclass_costs_dict[veh] = merge_object.get_markups(bca_tool.markups.loc[bca_tool.markups['fuelTypeID'] == veh[2], :])
        ...
        regclass_costs_dict[veh] = indirect_cost_obj.indirect_cost_scaled(regclass_costs_dict[veh], 'Warranty', bca_tool.warranty_vmt_share)
        regclass_costs_dict[veh] = indirect_cost_obj.indirect_cost_scaled(regclass_costs_dict[veh], 'RnD', bca_tool.r_and_d_vmt_share)
    ...
    for veh in alt_st_rc_ft_vehicles:
        if veh[3] == 2:
            ...
            def_costs_obj = DEFCost(veh, cost_steps, def_costs_dict[veh], bca_tool.def_doserate_inputs, bca_tool.def_gallons_perTonNOxReduction, bca_tool.def_prices)
            print(def_costs_obj)
    ...
    orvr_adjust_obj = ORVRadjust(alt_st_rc_ft_vehicles, bca_tool.orvr_fuelchanges, sourcetype_gallons,
    ...
    weighted_results_obj = WeightedResult(sourcetype_all_costs, 'VMT_AvgPerVeh', alt_st_rc_ft_vehicles, 'modelYearID',
                                          bca_tool.weighted_operating_cost_years, bca_tool.max_age_included, bca_tool.options_dict)
    ...
    for dr in [0, bca_tool.discrate_social_low, bca_tool.discrate_social_high]:
        discounting_obj = DiscountValues(sourcetype_all_costs, bca_metrics_to_discount, bca_tool.discount_to_yearID, bca_tool.costs_start)
    ...
    if bca_tool.calc_pollution_effects == 'Y':
        for col in criteria_costs_list_3:
            bca_dict[0.07][col] = np.nan
        for col in criteria_costs_list_7:
            bca_dict[0.03][col] = np.nan
    ...
    if bca_tool.calc_pollution_effects == 'Y':
        for mort_est in ['low', 'high']:
            bca.loc[bca['DiscountRate'] == 0.03, 'TotalCost_' + mort_est + '_' + str(0.03)] \
                = bca[['TechCost_TotalCost', 'OperatingCost_BCA_TotalCost', 'CriteriaCost_' + mort_est + '_' + str(0.03)]].sum(axis=1)
            bca.loc[bca['DiscountRate'] == 0.03, 'TotalCost_' + mort_est + '_' + str(0.07)] \
                = np.nan
            bca.loc[bca['DiscountRate'] == 0.07, 'TotalCost_' + mort_est + '_' + str(0.07)] \
                = bca[['TechCost_TotalCost', 'OperatingCost_BCA_TotalCost', 'CriteriaCost_' + mort_est + '_' + str(0.07)]].sum(axis=1)
            bca.loc[bca['DiscountRate'] == 0.07, 'TotalCost_' + mort_est + '_' + str(0.03)] \
                = np.nan

    # Now add an OptionName column so that output files provide that information
    Fleet(bca).insert_option_name(bca_tool.options_dict, number_alts)
    ...
    for group in ['yearID', 'yearID_rc_ft']:
        bca_summary[group] = DiscountValues(bca_summary[group], bca_metrics_to_cumsum, bca_tool.discount_to_yearID, bca_tool.costs_start).annualize()
    ...
    bca_tool.path_outputs.mkdir(exist_ok=True)
    path_of_run_folder = bca_tool.path_outputs.joinpath(f'{bca_tool.start_time_readable}_CTI_{bca_tool.run_folder_identifier}')
    ...
    discount_rates = [0]
    techcost_years = bca_tool.techcost_summary_years
    ...
    bca_years = bca_tool.bca_summary_years
    ...
    if bca_tool.calc_pollution_effects == 'Y':
        DocTables(bca_summary['yearID']).bca_yearID_tables('', 0, bca_years, 'billions', bca_cols, bca_annual,
                                                           'CriteriaCost_low_0.07', 'CriteriaCost_high_0.03')
    else:
        DocTables(bca_summary['yearID']).bca_yearID_tables('', 0, bca_years, 'billions', bca_cols, bca_annual)
    bca_annual.save()
    ...
    if bca_tool.calc_pollution_effects == 'Y':
        DocTables(bca_summary['yearID']).bca_yearID_tables('_CumSum', 0.03, bca_years, 'billions', bca_cols, bca_npv,
                                                           'CriteriaCost_low_0.03', 'CriteriaCost_high_0.03')
        DocTables(bca_summary['yearID']).bca_yearID_tables('_CumSum', 0.07, bca_years, 'billions', bca_cols, bca_npv,
                                                           'CriteriaCost_low_0.07', 'CriteriaCost_high_0.07')
    else:
        DocTables(bca_summary['yearID']).bca_yearID_tables('_CumSum', 0.03, bca_years, 'billions', bca_cols, bca_npv)
        DocTables(bca_summary['yearID']).bca_yearID_tables('_CumSum', 0.07, bca_years, 'billions', bca_cols, bca_npv)
    bca_npv.save()
    ...
    if bca_tool.calc_pollution_effects == 'Y':
        DocTables(bca_summary['yearID']).bca_yearID_tables('_Annualized', 0.03, bca_years, 'billions', bca_cols, bca_annualized,
                                                           'CriteriaCost_low_0.03', 'CriteriaCost_high_0.03')
        DocTables(bca_summary['yearID']).bca_yearID_tables('_Annualized', 0.07, bca_years, 'billions', bca_cols, bca_annualized,
                                                           'CriteriaCost_low_0.07', 'CriteriaCost_high_0.07')
    else:
        DocTables(bca_summary['yearID']).bca_yearID_tables('_Annualized', 0.03, bca_years, 'billions', bca_cols, bca_annualized)
        DocTables(bca_summary['yearID']).bca_yearID_tables('_Annualized', 0.07, bca_years, 'billions', bca_cols, bca_annualized)
    ...
    if bca_tool.calc_pollution_effects == 'Y':
        ...
        inventory_years = bca_tool.bca_summary_years
    ...
    inputs_filename_list = gen_fxns.inputs_filenames(bca_tool.input_files_pathlist)

    for file in inputs_filename_list:
        path_source = bca_tool.path_inputs.joinpath(file)
    ...
    fuel_prices.to_csv(path_of_modified_inputs_folder.joinpath('fuel_prices_' + bca_tool.aeo_case + '.csv'), index=False)
    bca_tool.regclass_costs.to_csv(path_of_modified_inputs_folder.joinpath('regclass_costs.csv'), index=False)
    markup_scalers_reshaped.to_csv(path_of_modified_inputs_folder.joinpath('markup_scalers_reshaped.csv'), index=False)
    bca_tool.repair_and_maintenance.to_csv(path_of_modified_inputs_folder.joinpath('repair_and_maintenance.csv'))
    bca_tool.def_prices.to_csv(path_of_modified_inputs_folder.joinpath('def_prices.csv'), index=False)
    ...
    for alt_num in range(1, len(bca_tool.options_dict)):
        k = alt_num * 10
        alt0 = bca_tool.options_dict[0]['OptionName']
        alt = bca_tool.options_dict[alt_num]['OptionName']
        bca_tool.options_dict.update({k: {'OptionName': f'{alt}_minus_{alt0}'}})

    if bca_tool.generate_emissionrepair_cpm_figures != 'N':
        cpm_figure_years = bca_tool.generate_emissionrepair_cpm_figures.split(',')
        ...
        for veh_name in veh_names:
            for cpm_figure_year in cpm_figure_years:
                CreateFigures(bca, bca_tool.options_dict, path_figures).line_chart_vs_age(0, alts, cpm_figure_year, veh_name, 'EmissionRepairCost_Owner_AvgPerMile')

    if bca_tool.generate_BCA_ArgsByOption_figures == 'Y':
        ...
        alts = pd.Series(bca.loc[bca['optionID'] >= 10, 'optionID']).unique()
        for alt in alts:
            CreateFigures(bca_summary['yearID'], bca_tool.options_dict, path_figures).line_chart_args_by_option(0, alt, yearID_min, yearID_max,
                                                                                                                'TechCost_TotalCost',
    ...
    if bca_tool.generate_BCA_ArgByOptions_figures == 'Y':
        yearID_min = int(bca['yearID'].min())
        yearID_max = int(bca['yearID'].max())
        path_figures = path_of_run_results_folder.joinpath('figures')
        path_figures.mkdir(exist_ok=True)
        ...
        for arg in args:
            CreateFigures(bca_summary['yearID'], bca_tool.options_dict, path_figures).line_chart_arg_by_options(0, alts, yearID_min, yearID_max, arg)

    if bca_tool.create_all_files == 'y' or bca_tool.create_all_files == 'Y' or bca_tool.create_all_files == '':
    ...
    elapsed_time = end_time - bca_tool.start_time
    ...
    summary_log = pd.DataFrame(data={'Item': ['Version', 'Run folder', 'Start of run', 'Elapsed time read inputs', 'Elapsed time calculations', 'Elapsed time save outputs', 'End of run', 'Elapsed runtime'],
                                     'Results': [project_code.__version__, path_of_run_folder, bca_tool.start_time_readable, bca_tool.elapsed_time_read, elapsed_time_calcs, elapsed_time_outputs, end_time_readable, elapsed_time],
                                     'Units': ['', '', 'YYYYmmdd-HHMMSS', 'seconds', 'seconds', 'seconds', 'YYYYmmdd-HHMMSS', 'seconds']})
    summary_log = pd.concat([summary_log, gen_fxns.get_file_datetime(bca_tool.input_files_pathlist)], axis=0, sort=False, ignore_index=True)
